                ...
                for language in languages:
                    ...
                    for child in language:
                        if child.tag == "sphinx-name":
                            ...
                            tojupyter_lang = child.text

                    if sphinx_lang and tojupyter_lang:
                        self.translator[sphinx_lang] = tojupyter_lang
                    else:
...
class JupyterOutputCellGenerators(Enum):
    ...
    def GetGeneratorFromClasses(obj, node):
        ...
        for item in class_list:
            if item == "no-execute" and not obj.tojupyter_ignore_no_execute:
                res["type"] = JupyterOutputCellGenerators.MARKDOWN
            elif item == "skip-test" and not obj.tojupyter_ignore_skip_test:
                res["type"] = JupyterOutputCellGenerators.MARKDOWN
            elif item == "output":
