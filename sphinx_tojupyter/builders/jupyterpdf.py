...
class JupyterPDFBuilder(Builder):
    ...
    def init(self):
        ...
        if  self.config["tojupyter_pdf_book"] and ("tojupyter_pdf_book_index" not in self.config or not self.config["tojupyter_pdf_book_index"]):
            self.logger.warning(
        ...
        if ("tojupyter_execute_notebooks" in self.config and self.config['tojupyter_execute_notebooks'] is False) or "tojupyter_execute_notebooks" not in self.config:
            self.config['tojupyter_execute_notebooks'] = True
            self.logger.info("execution of notebooks is mandatory for pdf conversion, so setting it on for pdf builder.")

        if ("tojupyter_target_pdf" in self.config and self.config['tojupyter_target_pdf'] is False) or "tojupyter_target_pdf" not in self.config:
            self.config['tojupyter_target_pdf'] = True
    ...
    def finish(self):
        ...
        if "tojupyter_target_pdf" in self.config and self.config["tojupyter_target_pdf"] and self.config["tojupyter_pdf_book"]:
