...
class MakePDFWriter():
    ...
    def convert_to_latex(self, builder, filename, latex_metadata):
        ...
        template_folder = builder.config['tojupyter_template_path']
        ...
        fl_tex_template = builder.confdir + "/" + template_folder + "/" + builder.config['tojupyter_latex_template']

        ## do not convert excluded patterns to latex
        excluded_files = [x in filename for x in builder.config['tojupyter_pdf_excludepatterns']]
    ...
    def nbconvert_index(self, builder):
        ...
        template_folder = builder.config['tojupyter_template_path']
        fl_tex_template = builder.confdir + "/" + template_folder + "/" + builder.config['tojupyter_latex_template_book']
    ...
    def process_tex_for_book(self, builder):
        ...
        if "tojupyter_pdf_book_name" in builder.config and builder.config["tojupyter_pdf_book_name"]:
            filename = builder.config["tojupyter_pdf_book_name"]
